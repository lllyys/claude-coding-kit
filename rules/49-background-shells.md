...
The long-runners these rules apply to: a dev/watch server (the project's run-the-app command), watch-mode test runs, full builds, the project's test command on a large suite, and Codex audit runs. A dev/watch server in particular is **intentionally long-lived** — if you start it for verification, treat it as an explicitly-owned job (case (b) below), not a waiter, and stop it when the verification slice is done.
...
```bash
# Launched as run_in_background after kicking off a long test run
# (TEST_CMD = the project's test command, e.g. from
# .claude/tdd-guardian/config.json → testCommand):
until ! pgrep -f "$TEST_CMD" >/dev/null 2>&1; do sleep 5; done

# And in another tab:
while pgrep -f "$TEST_CMD" >/dev/null 2>&1; do sleep 10; done
echo "---done---"
tail -50 /private/tmp/.../<launch>.output
```

The waiters keyed on the predicate `pgrep -f "$TEST_CMD"` — the test command's class, not the specific test process. The original test finished cleanly at 19:39, but every subsequent run of the same test command later in the session (a bug-fix test gate at 21:32, the verification run at 22:00) re-triggered the predicate. The loops never exited, and Claude's task UI showed them as "running" while OS-level `ps` showed nothing.
...
2. **Never use `pgrep -f` against a generic command name as a gate.** `pgrep -f "<test-command>"` matches the class, not the instance. A later run of the same tool will resurrect the predicate.
...
```bash
# <DONE-MARKER> = a string the test runner prints exactly once at the end
# of THIS run (e.g. its summary line), or a run-specific sentinel you echo.
until grep -q "<DONE-MARKER>" "$LOG" 2>/dev/null; do sleep 5; done
# OR
...
|---|---|---|
| `until ! pgrep -f "<test-command>"; do sleep 5; done` | Matches a CLASS of work; future invocations re-arm the wait | `wait $!` or sentinel grep |
...
- If a long test gate IS in flight, prefer one of these closures:
  - `<test-command> 2>&1 | tail -25` foreground in the iteration's terminal step (slow but unambiguous), OR
