...
description: "Run the binding 6-gate feature workflow end-to-end per rule 47 (Plan → Independent Plan Audit → TDD Implementation → Implementation Audit Loop → Integration Verification → Merge). Use this skill whenever the user wants to implement a new feature, asks 'implement feature #N', 'work on feature 47', 'start the feature workflow', 'plan + build feature X', or describes building a new capability that doesn't yet exist. NOT for fixing broken implementations (that's fix-issue). The skill drives the row from `TODO` → `VERIFIED` through six gates that must never be skipped — author/auditor separation, evidence files, hook compliance are all binding."
---
...
> Audit Loop → Integration Verification → Merge → (close gate)
...
1. Model assumption verification — do the types, data shapes,
   interface methods, function signatures, and file paths the plan
   names actually exist in the current codebase? (This catches the
   largest class of pre-implementation bugs.)
2. Risks + missing edge cases — what failure modes the plan misses.
3. Interface signature critique — are the new interfaces the plan
   introduces well-shaped, or do they leak implementation concerns?
4. Concurrency / state hazards — race conditions, ordering bugs,
   stale references, cancellation/cleanup handling.
...
6. Foundational-vs-behavioral classification — is each WI's tier
   ...
   user-observable behavior. Behavioral = anything that changes
   observable app behavior, persistent state, or a user-facing flow.)
...
1. **RED** — write a failing test (using the project's test framework)
   that captures the WI's behavior. Match the test shape to the unit:
   - Stateful module / store → test that drives the public actions and
     asserts on resulting state, with state reset between cases
   - I/O or integration boundary → test against a mocked transport /
     fake dependency, with no real network or external service calls
   - UI component / view → test through the project's UI test harness,
     querying by user-facing identifiers (role/name/label) where it has one
   - Pure utility → table-driven / parameterized test
...
Run the project's build/gate command (see `.claude/rules/40-version-bump.md`
and `.claude/tdd-guardian/config.json`). The release-gate skill runs
whatever the project configures — typically lint → tests + coverage →
build chained together.

Pass → continue. Fail → fix and retry. 3 failures → stop, report.

> **Note**: the project's build/gate command is the binding quality
> gate. During the RED→GREEN loop you can iterate faster by running just
> the WI's spec via the project's test command
> (see `.claude/tdd-guardian/config.json` → `testCommand`), but the WI is
> not done until the full gate (lint + coverage + build) is green.
...
2. Edge cases — boundary conditions, null/undefined, empty input,
   Unicode / non-ASCII text, cancellation mid-operation, concurrent
   state updates
3. Security — injection / unsafe rendering of untrusted input, input
   sanitization, no secrets (API keys, tokens) leaked to logs or any
...
7. Project conventions — language/type strictness (no escape hatches
   that defeat the type checker), resource cleanup, no stale references,
   file size <300 lines (see `.claude/rules/50-codebase-conventions.md`)
8. Boundary discipline — feature code goes through the project's
   designated abstraction layers rather than reaching past them into
   third-party SDKs directly; long-running / async operations handle
...
|---|---|
| New module boundary / shared abstraction / cross-feature pattern | `docs/architecture.md` |
...
Bump the semver version wherever this project records it — its manifest
(e.g. `package.json`, `pyproject.toml`, `Cargo.toml`, a `VERSION` file,
etc.). The steps are manifest-agnostic:

```bash
# 1. Bump the version field to X.Y.Z in the project's manifest.
# 2. Confirm the field updated (grep the manifest for the new version).
# 3. Run the project's build/gate command as a smoke check.
# 4. Stage the manifest and commit:
git add <manifest>
...
- [x] Project build/gate command passes (Gate 3 test gate)
...
| **Behavioral** (anything that changes observable app behavior, persistent state, or a user-facing flow) | **Slice verification** — run the app the way this project runs it — CLI, server, desktop, browser, or a library test harness, whatever applies — and exercise the slice end-to-end; automate with the project's E2E tool if it has one. Point at real-enough inputs (a local backend, a mocked transport, fixture data) and observe the actual behavior. | PR description "Gate 5a Verification" section: what was run, what was observed |
| **Final WI** (the one that completes the feature) | **Pre-merge slice** of the final acceptance criteria — exercise what's exercisable on the working-tree build. Defer anything that requires a merged-on-main build. | PR description "Gate 5a Verification" + a note "5b post-merge evidence file pending" |
...
app_version: <version from the project manifest>
date: YYYY-MM-DD
verifier: <name or "claude">
environment: <how the app was exercised, e.g. "CLI on macOS 15.4", "Chromium 126 (Playwright)", "server + curl">
os_version: <e.g. "macOS 15.4">
build_mode: <how it was built/run, e.g. dev | preview | production | release>
dependencies: <relevant external deps or services, e.g. "local backend", "mocked transport", or "n/a">
...
- `## Commands run` — fenced code blocks of the actual commands used
  (shell, test runner, E2E tool, HTTP calls). Reproducible recipe.
- `## Observations` — what surprised you, what was almost a
  regression, what's brittle for next time.
- `## Artifacts` — paths to screenshots, traces, log captures.
...
> a specific tool is named and confirmed missing (e.g. the project's
> test command returns "command not found", the E2E tool's browsers
> aren't installed, a required backend/service is down). "I'll do it
> next session" is not a tool-unavailability claim — it's a discipline
> lapse.
...
1. Apply label to GH issue (if verifiable by running the app, default
   `awaiting-verification`; otherwise pick from
...
- For the **run-the-app verification** path:
  - Check out and run the merged build the way this project runs — CLI,
    server, desktop, browser, or a library test harness, whatever applies.
  - Run every acceptance criterion from the plan; automate with the
    project's E2E tool where possible.
  - Record observations in `dev-docs/verification/feature-<id>-<YYYYMMDD>.md`.
  - Post closure comment citing the evidence file:

    ```
    gh issue comment <feature-gh-issue> --body "VERIFIED by running the merged build (commit <sha>). All acceptance criteria pass. Evidence: dev-docs/verification/feature-<id>-<YYYYMMDD>.md."
    gh issue close <feature-gh-issue>
    ```

- For features whose failure mode **cannot be reproduced by running the
  app** (race conditions, fault injection, etc.): close under
  `verification-exception` with a high-fidelity integration test at real
  subsystem boundaries (not stubs). Same evidence file, citation comment,
  then `gh issue close`.
