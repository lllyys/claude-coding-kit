...
5) **Test (Test Runner agent)**
   - Run the project's build/gate command (the release-gate skill runs whatever the project configures — typically lint -> tests/coverage -> build).
   - If user-facing flows are impacted, verify by running the app the way this project runs it (CLI, server, desktop, browser, or a library test harness, whatever applies) and exercising the flow; automate with the project's E2E tool if it has one.
